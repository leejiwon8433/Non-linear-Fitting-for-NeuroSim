...
namespace NonLinearFitter_NeuroSim {
  internal class NonLinearFitter {
    ...
    public NonLinearFitter(List<Point> ltps, List<Point> ltds,
                           double cycleToCycleVariationLTP = 0,
                           double cycleToCycleVariationLTD = 0,
                           double randomMin = -2.3,
                           double randomMax = 2.3) {
      LTPs = ltps;
      LTDs = ltds;
      RandomMin = randomMin;
      RandomMax = randomMax;
    ...
    public Result Fit() {
      ...
      for (int i = 0; i < fittedLTDs.Count; i++)
        fittedLTDs[i].X = (double)i / (fittedLTDs.Count - 1);

      result.FittedLTDs = fittedLTDs;
